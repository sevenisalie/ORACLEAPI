 ...
 */
export function createRemapImportsTransformer({ remapTarget }) {
    return context => sourceFile => remapSourceFileImports(sourceFile, context, remapTarget);
}
export function remapSourceFileImports(sourceFile, context, remapTarget) {
    ...
    function visitStaticImportsExports(node) {
        if (ts.isImportDeclaration(node) && ts.isStringLiteral(node.moduleSpecifier)) {
            ...
            if (originalTarget !== remappedTarget) {
                return ts.updateImportDeclaration(node, node.decorators, node.modifiers, node.importClause, ts.createLiteral(remappedTarget));
            }
        }
            ...
            const remappedTarget = remapTarget(originalTarget, fileName, sourceFile);
            if (originalTarget !== remappedTarget) {
                return ts.updateExportDeclaration(node, node.decorators, node.modifiers, node.exportClause, ts.createLiteral(remappedTarget));
            }
        }
